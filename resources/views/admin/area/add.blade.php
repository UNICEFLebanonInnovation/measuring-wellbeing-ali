...
<div class="row">
	<div class="col-lg-12">
		...
		<div class="m-portlet">
			...
			<!--begin::Form-->
			<form class="m-form m-form--fit m-form--label-align-right m-form--group-seperator-dashed" id="add_caza" action="{{ url('add-area') }}" method="post">
				@csrf
				<div class="m-portlet__body">
					<div class="form-group m-form__group row">
						...
						<div class="col-lg-6">
							...
							<select class="form-control m-input" id="caza" name="caza">
								...
								{{-- @foreach($caza as $gov)
									<option value="{{ $gov->id }}" {{ old('caza') == $gov->id ? 'selected':'' }}>{{ ucfirst($gov->name) }}</option>
								@endforeach --}}
							</select>
							@if ($errors->has('name'))
			                  	<span class="m-form__help">{{ $errors->first('name') }}</span>
					...
					<div class="form-group m-form__group row">
						<div class="col-lg-6">
							...
							<input type="text" class="form-control m-input" value="{{ old('name') }}" id="name" name="name" placeholder="Enter Area name">
							@if ($errors->has('name'))
			                  	<span class="m-form__help">{{ $errors->first('name') }}</span>
			              	@endif
						</div>
